...
```sh
./target/release/dubhe-node --dev
```

To purge the development chain's state, run the following command:

```sh
./target/release/dubhe-node purge-chain --dev
```

To start the development chain with detailed logging, run the following command:

```sh
RUST_BACKTRACE=1 ./target/release/dubhe-node -ldebug --dev
```
...
$ ./target/release/dubhe-node --dev --base-path ./my-chain-state/
...
`pallets`](pallets/bridge/src/lib.rs) directory.
